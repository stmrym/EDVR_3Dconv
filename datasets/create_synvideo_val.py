...
[Output] validation_RR, validation_GT, validation_R_blur_mask
...
# create validation dataset from '210' to '230'
startdir = 210
lastdir = 230
...
for i in range(0,dirnum//2):
    ...
    for id in tqdm(range(0, images_len)):
        ...
        if ((t_image is not None) and (r_image is not None)):
            ...
            if is_saving == True:
                os.makedirs('validation_RR/' + save_id, exist_ok=True)
                plt.imsave("validation_RR/" + save_id + "/{:0=8}.png".format(count), blended)
                os.makedirs('validation_GT/' + save_id, exist_ok=True)
                plt.imsave("validation_GT/" + save_id + "/{:0=8}.png".format(count), out_t)
                os.makedirs('validation_R_blur_mask/' + save_id, exist_ok=True)
                plt.imsave('validation_R_blur_mask/' + save_id + "/{:0=8}.png".format(count), r_blur_mask)
