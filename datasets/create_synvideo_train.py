...
[Output] train_RR, train_GT, train_R_blur_mask
...
# create train dataset from '000' to '209'
startdir = 0
lastdir = 210
...
for i in range(0,dirnum//2):
    ...
    for id in tqdm(range(0, images_len)):
        ...
        if ((t_image is not None) and (r_image is not None)):
            ...
            if is_saving == True:
                os.makedirs('train_RR/' + save_id, exist_ok=True)
                plt.imsave("train_RR/" + save_id + "/{:0=8}.png".format(count), blended)
                os.makedirs('train_GT/' + save_id, exist_ok=True)
                plt.imsave("train_GT/" + save_id + "/{:0=8}.png".format(count), out_t)
                os.makedirs('train_R_blur_mask/' + save_id, exist_ok=True)
                plt.imsave('train_R_blur_mask/' + save_id + "/{:0=8}.png".format(count), r_blur_mask)
