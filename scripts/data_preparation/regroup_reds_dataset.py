...
def regroup_reds_dataset(train_path, val_path):
    ...
    for folder in val_folders:
        ...
        print(folder)
        print(os.path.join(train_path, str(new_folder_idx)))
        os.system(f'echo D | xcopy {folder} {os.path.join(train_path, str(new_folder_idx))}')    # cp -r より変更


if __name__ == '__main__':
    """    
    # train_sharp
    train_path = 'datasets\\REDS\\train_sharp'
    val_path = 'datasets\\REDS\\val_sharp'
    regroup_reds_dataset(train_path, val_path)

    # train_sharp_bicubic
    train_path = 'datasets\\REDS\\train_sharp_bicubic\\X4'
    val_path = 'datasets\\REDS\\val_sharp_bicubic\\X4'
    regroup_reds_dataset(train_path, val_path)
    """
    # train_blur
    train_path = 'datasets\\REDS\\train_blur'
    val_path = 'datasets\\REDS\\val_blur'
    regroup_reds_dataset(train_path, val_path)
